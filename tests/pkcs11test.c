...
static CK_RV test_x509_cert_search_crash(void* args)
{
    CK_SESSION_HANDLE session = *(CK_SESSION_HANDLE*)args;
    CK_RV ret = CKR_OK;
    ...
    CK_BYTE abyTemplate1[] = { 0x1 };
        ...
        */
        ...
        CK_BYTE abyTemplate9[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };

        CK_BYTE abyTemplate10[] = { 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };
        ...
        CK_ATTRIBUTE createAttributes[]
            = { { 0x11, abyTemplate0, 915 },    // CKA_VALUE
                ...
                { 0x80, abyTemplate9, 8 },      // CKA_CERTIFICATE_TYPE
                { 0x0, abyTemplate10, 8 },      // CKA_CLASS
              };

    /* Create the certificate object */
    ret = funcList->C_CreateObject(session, createAttributes,
                                   sizeof(createAttributes) / sizeof(CK_ATTRIBUTE), &hTest);
    CHECK_CKR(ret, "Create certificate object");

    if (ret == CKR_OK) {
        CK_BYTE abySearchTemplate0[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };
        ...
        CK_BYTE abySearchTemplate2[] = { 0x3, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };
        ...
        /* Initialize the search - this is where the crash might occur */
        ret = funcList->C_FindObjectsInit(session, aSearchAttributes,
                                          sizeof(aSearchAttributes) / sizeof(CK_ATTRIBUTE));
        CHECK_CKR(ret, "C_FindObjectsInit");

        if (ret == CKR_OK) {
            /* Search for objects - this call was crashing in customer code */
            while (ret == CKR_OK) {
                ret = funcList->C_FindObjects(session, &hObjectFound, 1, &ulObjectCount);
                CHECK_CKR(ret, "C_FindObjects");

                if (ret != CKR_OK || ulObjectCount == 0) {
                    break;
                }

                /* Get attributes of found object to verify it's accessible */
                ...
                ret = funcList->C_GetAttributeValue(session, hObjectFound, getAttributes, 4);
                if (ret == CKR_OK) {
                    /* Allocate memory for attribute values */
                    for (i = 0; i < 4; i++) {
                        if (getAttributes[i].ulValueLen > 0) {
                            getAttributes[i].pValue = XMALLOC(getAttributes[i].ulValueLen,
                                                              NULL, DYNAMIC_TYPE_TMP_BUFFER);
                        }
                    }
                    ...
                    for (i = 0; i < 4; i++) {
                        if (getAttributes[i].pValue != NULL) {
                            XFREE(getAttributes[i].pValue, NULL, DYNAMIC_TYPE_TMP_BUFFER);
                        }
                    }
                }
            }
...
static TEST_FUNC testFunc[] = {
    ...
    PKCS11TEST_FUNC_SESS_DECL(test_x509_cert_search_crash),
