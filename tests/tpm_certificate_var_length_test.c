...
#ifdef DEBUG_WOLFPKCS11
    wolfPKCS11_Debugging_On_fp = (void(*)(void))dlsym(dlib, "wolfPKCS11_Debugging_On");
    wolfPKCS11_Debugging_Off_fp = (void(*)(void))dlsym(dlib, "wolfPKCS11_Debugging_Off");
#endif
#else
...
static CK_RV tpm_certificate_var_length_test(void)
{
    ...
    /* Step 2: Close session and completely finalize PKCS11 to force storage flush */
    ...
    /* Completely finalize PKCS11 to ensure token data is written to storage */
    ret = pkcs11_final();
    if (ret != CKR_OK) {
        return ret;
    }

    fprintf(stderr, "Step 2b: Finalized PKCS11 completely to force storage flush\n");

    /* Step 3: Completely reinitialize PKCS11 and token to force read from storage */
    ret = pkcs11_init();
    if (ret != CKR_OK) {
        return ret;
    }

    fprintf(stderr, "Step 3: Reinitialized PKCS11 from storage\n");

    /* Step 4: Open new session and log back in */
    ret = pkcs11_open_session(&session2);
    if (ret != CKR_OK) {
        return ret;
    }

    fprintf(stderr, "Step 4: Logged back in with new session\n");

    /* Step 5: Find the certificate object */
    ...
    fprintf(stderr, "Step 5: Certificate found successfully\n");

    /* Step 6: Verify all variable length attributes are correct */
    ...
    fprintf(stderr, "Step 6: All variable length attributes verified successfully\n");
...
int main(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc; i++) {
        if (XSTRNCMP(argv[i], "-h", 2) == 0 || XSTRNCMP(argv[i], "--help", 6) == 0) {
            fprintf(stderr, "Usage: %s [options]\n", argv[0]);
            fprintf(stderr, "Options:\n");
            fprintf(stderr, "  -v        Verbose output\n");
            fprintf(stderr, "  -d        Enable wolfPKCS11 debugging (implies -v)\n");
            fprintf(stderr, "  -h,--help Show this help\n");
            fprintf(stderr, "\nNote: This test requires WOLFPKCS11_TPM and storage support\n");
            return 0;
        }
    }
    ...
    for (i = 1; i < argc; i++) {
        if (XSTRNCMP(argv[i], "-v", 2) == 0) {
            ...
            verbose = 1; /* Enable verbose when debug is on */
        }
    }
    ...
    /* Run the test - note: test handles its own finalize/reinitialize cycle */
...
int main(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc; i++) {
        if (XSTRNCMP(argv[i], "-h", 2) == 0 || XSTRNCMP(argv[i], "--help", 6) == 0) {
            ...
            fprintf(stderr, "  -h,--help Show this help\n");
            fprintf(stderr, "\nNote: This test requires WOLFPKCS11_TPM and storage support\n");
            fprintf(stderr, "Current build: TPM support not available\n");
            return 0;
        }
    }
